...
\begin{description}
  ...
  \item[Queueing] \texttt{FRM}, \texttt{FRMFW}, \texttt{NRM}, \texttt{Coevolve}
...
\begin{frame}{Queueing}{The \texttt{Coevolve} approach}
...
\begin{frame}{The compound Hawkes process benchmark}{Median execution time, 50 samples per node (V) x time}

\begin{tikzpicture}

  \node[anchor=south west] (image) at (0, 0) {\includegraphics[trim={0 0 0 0.8cm},clip,width=10cm]{assets/hawkes-benchmark.png}};
  \node[text width=8cm,overlay,yshift=4.5cm,xshift=-4.5cm,anchor=north west] at (image.south east) {%
    \footnotesize\texttt{use\_recursion=false} loops through the whole history of past events. \texttt{use\_recursion=true} only looks at the previous state of each node. \textit{Direct()} and \texttt{Coevolve} are algorithms from \texttt{JumpProcesses.jl}. When coupled with an ODE stepper, \texttt{Direct()} does not use its own algorithm, but the inverse method mentioned previously. \texttt{PDMPCHV()} is an algorithm from \texttt{PiecewiseDeterministicMarkovProcesses.jl} and \texttt{PyTick()} is a Python library.};
